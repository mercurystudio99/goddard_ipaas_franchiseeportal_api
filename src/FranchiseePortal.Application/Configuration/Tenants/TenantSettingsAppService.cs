...
using FranchiseePortal.Security;
using FranchiseePortal.Storage;
using FranchiseePortal.Timing;
...
namespace FranchiseePortal.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        private async Task<TenantOtherSettingsEditDto> GetOtherSettingsAsync()
        {
            ...
            {
                IsQuickThemeSelectEnabled = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.IsQuickThemeSelectEnabled),
                SystemAlertMessage = await SettingManager.GetSettingValueAsync(AppSettings.UserManagement.SystemAlertMessage)
            };
        }
        ...
        private async Task UpdateOtherSettingsAsync(TenantOtherSettingsEditDto input)
        {
            ...
            await SettingManager.ChangeSettingForTenantAsync(
                AbpSession.GetTenantId(),
                AppSettings.UserManagement.SystemAlertMessage,
                input.SystemAlertMessage
            );
        }
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !FranchiseePortalConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
