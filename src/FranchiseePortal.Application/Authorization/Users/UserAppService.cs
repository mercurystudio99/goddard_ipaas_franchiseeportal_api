...
using FranchiseePortal.Authorization.Permissions;
...
using FranchiseePortal.Authorization.Users.Exporting;
...
namespace FranchiseePortal.Authorization.Users
{
    ...
    public class UserAppService : FranchiseePortalAppServiceBase, IUserAppService
    {
        ...
        public async Task<FileDto> GetUsersToExcel(GetUsersToExcelInput input)
        {
            ...
            var userListDtos = ObjectMapper.Map<List<UserListDto>>(users);
            await FillRoleNames(userListDtos);
            await FindLastLoginDates(userListDtos);
        ...
        private async Task FindLastLoginDates(List<UserListDto> userListDtos)
        {
            var userIds = userListDtos.Select(u => u.Id);

            var loginAttemps = await _userLoginAttemptRepository
                    .GetAll()
                    .Where(la => userIds.Contains(la.UserId.GetValueOrDefault()) && la.Result == AbpLoginResultType.Success)
                    .GroupBy(la => la.UserId)
                    .Select(u => new
                    {
                        UserId = u.Key,
                        LastLoginDate = u.Max(x => x.CreationTime)
                    })
                    .ToListAsync();

            foreach (var userListDto in userListDtos)
            {
                userListDto.LastLoginDateTime = loginAttemps.FirstOrDefault(l => l.UserId == userListDto.Id)?.LastLoginDate;
            }
        }
