...
using FranchiseePortal.Web.Authentication.JwtBearer;
using FranchiseePortal.Web.Authentication.TwoFactor;
using FranchiseePortal.Web.Models.TokenAuth;
using FranchiseePortal.Authorization.Impersonation;
using FranchiseePortal.Authorization.Roles;
using FranchiseePortal.Configuration;
using FranchiseePortal.Identity;
using FranchiseePortal.Net.Sms;
using FranchiseePortal.Notifications;
using FranchiseePortal.Security.Recaptcha;
using FranchiseePortal.Web.Authentication.External;
...
namespace FranchiseePortal.Web.Controllers
{
    ...
    public class TokenAuthController : FranchiseePortalControllerBase
    {
        ...
        {
            ExternalAuthUserInfo externalUser = null;
            try
            {
            ...
            {
                // If the token is expired, this will throw an exception and return a 500 unless we handle the
                // underlying unauthorized exception and convert it to an Abp Exception 20220912TJA
                if (hex.StatusCode == HttpStatusCode.Unauthorized)
                {
                    var msg = $"Unauthorized token use, returning 401";

                    throw new AbpAuthorizationException(msg, hex);
                }
            ...
            switch (loginResult.Result)
            {
                ...
                case AbpLoginResultType.UnknownExternalLogin:
                {
                    ...
                    if (user != null)
                    {
                        // User already exists by email, add external login to logins
                        _userStore.AddLogin(user, new UserLoginInfo(externalUser.Provider, externalUser.ProviderKey, externalUser.Provider));

                        await CurrentUnitOfWork.SaveChangesAsync();
                    }
                    else
                    {
                        user = await RegisterExternalUserAsync(externalUser);
                    }
        ...
        private async Task<User> RegisterExternalUserAsync(ExternalAuthUserInfo externalLoginInfo)
        {
            ...
            var user = await _userRegistrationManager.RegisterAsync(
                ...
                externalLoginInfo.Surname ?? String.Empty,
